...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=playfair-display:700|great-vibes:400" rel="stylesheet" />
>>>>>>> 993b315 (Manu):SeduhRasa1/resources/views/home.blade.php
</head>
...
=======
    ...
    <section class="relative h-[70vh] md:h-[80vh] w-full overflow-hidden">
        @php
            $hero =
                isset($site) && !empty($site?->hero_image)
                    ? asset('storage/' . $site->hero_image)
                    : 'https://images.unsplash.com/photo-1511920170033-f8396924c348?q=80&w=1600&auto=format&fit=crop';
        @endphp
        ...
        <div
            class="relative z-10 max-w-4xl mx-auto h-full px-4 flex flex-col items-center justify-center text-center text-white">
            <p class="tracking-[0.35em] uppercase text-amber-300 text-xs md:text-sm mb-3">Best Coffee Shop</p>
            <h1 class="font-['Great_Vibes'] text-4xl md:text-6xl leading-tight">
                Coffee from the Best Sunny<br />Plantations
            </h1>
            <a href="#product"
                class="mt-6 inline-block bg-amber-500 hover:bg-amber-600 text-stone-900 font-semibold rounded px-5 py-2">
                Shop Now
            </a>
        </div>
        ...
        {{-- arrow kiri/kanan (dummy) --}}
        <button
            class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 grid place-items-center rounded-full bg-white/20 text-white hover:bg-white/30">‹</button>
        <button
            class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 grid place-items-center rounded-full bg-white/20 text-white hover:bg-white/30">›</button>
    </section>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach ($products as $p)
                    <div class="bg-white rounded-lg shadow border overflow-hidden hover:shadow-md transition">
                        ...
                        <div class="p-4 space-y-2">
                            {{-- rating bintang (opsional, tetap ada) --}}
                            <div class="flex items-center gap-1 text-amber-500">
                                @for ($i = 1; $i <= 5; $i++)
                                    @if ($i <= $p['rating'])
                                    ...
                                    @else
                                        <svg class="w-4 h-4 text-stone-300" viewBox="0 0 20 20" fill="currentColor">
                                            <path
                                                d="M10 15.27l5.18 3.05-1.64-5.81 4.46-3.86-5.85-.5L10 2 7.85 8.15l-5.85.5 4.46 3.86-1.64 5.81z" />
                                        </svg>
                                    @endif
                            ...
                            {{-- Bagian harga DIHAPUS. Tombol Add tetap ada. --}}
                            <div class="pt-2">
                                <button class="w-full py-2 text-sm rounded bg-stone-900 text-white hover:bg-stone-800">
                                    Add
                                </button>
                            </div>
                        </div>
