...
<body class="bg-stone-50 text-stone-900">
    ...
    <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between text-sm">
      <div class="flex items-center gap-6">
        ...
        <div class="hidden md:flex items-center gap-5 text-white/90">
          ...
          <a href="{{ url('/services') }}" class="hover:text-white">Services</a>
          <a href="{{ url('/contact') }}" class="hover:text-white">Contact</a>
        </div>
      </div>

      <div class="hidden md:flex items-center gap-2 text-white/90">
        {{-- PERBAIKAN: Ganti route() ke url() supaya tidak error --}}
        <a href="{{ url('/order') }}" class="px-3 py-1.5 rounded bg-amber-500 hover:bg-amber-600 text-stone-900 font-medium">
          Order
        </a>
      </div>
    </div>
  ...
  <section class="relative h-[70vh] md:h-[80vh] w-full overflow-hidden">
    <img class="hero-slide absolute inset-0 w-full h-full object-cover opacity-100 transition-opacity duration-700" src="{{ asset('welcome(1).jpg') }}" alt="Hero 1">
    <img class="hero-slide absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-700"  src="{{ asset('welcome(2).jpg') }}" alt="Hero 2">
    <img class="hero-slide absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-700"  src="{{ asset('welcome4.jpg') }}"   alt="Hero 3">
  ...
  <section id="about" class="py-16 bg-white">
    <div class="max-w-6xl mx-auto px-4 text-center">
      <h2 class="font-['Great_Vibes'] text-4xl md:text-5xl mb-6 text-stone-800">Tentang Kami</h2>
      <p class="max-w-2xl mx-auto text-stone-600 mb-10">
        SeduhRasa lahir dari kecintaan terhadap kopi dan keinginan untuk berbagi rasa. Kami bukan hanya penyaji kopi, tapi pembawa cerita, inspirasi, dan energi dalam setiap seduhan.
      </p>
    ...
    [
      ...
      'ig'  => 'https://www.instagram.com/arifinilham'
    ],
    [
      'name' => 'Mazira',
      'img' => 'https://cdn-icons-png.flaticon.com/512/4140/4140051.png',
      'ig'  => 'https://www.instagram.com/mazira'
    ],
    [
      'name' => 'Markus Edison',
      'img' => 'https://cdn-icons-png.flaticon.com/512/4140/4140037.png',
      'ig'  => 'https://www.instagram.com/markusedison'
    ],
    [
      'name' => 'Elsa Syafitriani',
      'img' => 'https://cdn-icons-png.flaticon.com/512/4140/4140051.png',
      'ig'  => 'https://www.instagram.com/elsasyafitriani'
    ],
  ];
@endphp

       @foreach($team as $member)
  <a href="{{ $member['ig'] }}" target="_blank" class="block bg-stone-50 p-6 rounded-xl shadow hover:shadow-md transition hover:bg-amber-50">
    <img src="{{ $member['img'] }}" class="w-24 h-24 mx-auto rounded-full mb-4" alt="{{ $member['name'] }}">
    <h3 class="text-lg font-semibold">{{ $member['name'] }}</h3>
    <p class="text-sm text-amber-600 mt-1">@{{ basename($member['ig']) }}</p>
  </a>
@endforeach
